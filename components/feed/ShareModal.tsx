...
// Defined at module level so React never treats it as a new component type mid-render,
// which would cause unmount/remount and break slider dragging.
...
export default function ShareModal({ pkg, open, onClose, momentId, mediaAssets, cta, ctaUrl, momentDifferentiators, onShared }: Props) {
  ...
  // Fresh voice profile fetched from API (not stale session cache)
  ...
  const displayCopy = copy.replace('{{SHARE_URL}}', shareUrl || '[link]')
  const charCount = displayCopy.length

  // Fetch fresh profile when modal opens to get latest voice_profile
  useEffect(() => {
    if (!open) return
    fetch('/api/profile')
      .then((r) => r.json())
  ...
  const generateCopy = useCallback(async () => {
    ...
    try {
      ...
      setCopy(pkg.example_copies?.[0] || `Just discovered something worth sharing — ${pkg.title}. {{SHARE_URL}}`)
    } finally {
      setGenerating(false)
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [platform, pkg.id, pkg.title, pkg.body, pkg.description, pkg.example_copies, formalityScore, voiceInfluenceScore, audienceScore, lengthTarget, selectedDiffs, variationIndex, voiceProfile])
  ...
  useEffect(() => {
    if (open) { generateCopy(); loadDriveFiles() }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [open, platform])
  ...
  async function handleShare(p: Platform) {
    try {
      ...
      const data = await res.json()
      const resolvedUrl = data.shareUrl || process.env.NEXT_PUBLIC_APP_URL || 'https://www.intercom.com'
      setShareUrl(resolvedUrl)
      const finalCopy = copy.replace('{{SHARE_URL}}', resolvedUrl)
      const shareHref = p === 'linkedin' ? buildLinkedInShareUrl(finalCopy, resolvedUrl) : buildXShareUrl(finalCopy)

      // 1. Copy to clipboard
      try { await navigator.clipboard.writeText(finalCopy) } catch { /* ignore */ }

      // 2. Show "copied" overlay
      setCopiedOverlay(true)

      // 3. After 500ms open the platform window, then switch to success overlay
      setTimeout(() => {
      ...
      const finalCopy = copy.replace('{{SHARE_URL}}', process.env.NEXT_PUBLIC_APP_URL || 'https://www.intercom.com')
      try { await navigator.clipboard.writeText(finalCopy) } catch { /* ignore */ }
      const shareHref = p === 'linkedin' ? buildLinkedInShareUrl(finalCopy) : buildXShareUrl(finalCopy)
      window.open(shareHref, '_blank', 'width=620,height=600,noopener,noreferrer')
    }
  }

  async function copyToClipboard() {
    await navigator.clipboard.writeText(copy.replace('{{SHARE_URL}}', shareUrl || process.env.NEXT_PUBLIC_APP_URL || ''))
    setCopied(true)
  ...
  return (
    <Dialog.Root open={open} onOpenChange={(v) => { if (!v) onClose() }}>
      <Dialog.Portal>
        ...
        >
          {/* Overlays: copied notice → success */}
          <AnimatePresence>
            {copiedOverlay && (
              <motion.div key="copied" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                ...
                <motion.div initial={{ scale: 0.8 }} animate={{ scale: 1 }} transition={{ type: 'spring', stiffness: 300, damping: 20 }}>
                  <div className="flex flex-col items-center gap-4">
                    ...
                    <p className="text-muted-foreground">Opening LinkedIn now…</p>
                  </div>
                </motion.div>
              </motion.div>
            )}
              ...
              <motion.div key="shared" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                ...
                <motion.div initial={{ scale: 0.5 }} animate={{ scale: 1 }} transition={{ type: 'spring', stiffness: 300, damping: 20 }}>
                  <div className="flex flex-col items-center gap-4">
                    ...
                    <p className="text-muted-foreground">Your post is on its way to your audience.</p>
                  </div>
                </motion.div>
              </motion.div>
            )}
          ...
          <div className="flex items-center justify-between px-8 py-4 border-b border-border bg-card shrink-0">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-0.5">Craft your post, then share it in one click</p>
            </div>
          ...
          <div className="flex flex-1 overflow-hidden">
            ...
            <div className="w-64 shrink-0 border-r border-border flex flex-col bg-background/40 overflow-y-auto">
              <div className="p-4 space-y-4">
                ...
                {/* CTA — most important context, shown first */}
                {(cta || ctaUrl) && (
                  <div>
                    ...
                    <div className="text-xs text-foreground leading-relaxed bg-secondary rounded-xl p-3 border border-border space-y-1.5">
                      {briefExpanded || pkg.body.length <= BRIEF_LIMIT ? (
                        <>
                          ...
                          {pkg.body.length > BRIEF_LIMIT && (
                            <button onClick={() => setBriefExpanded(false)}
                              className="text-muted-foreground text-[10px] hover:text-foreground transition-colors mt-0.5">
                              Collapse ↑
                            </button>
                          )}
                        </>
                        ...
                        <>
                          ...
                          <button onClick={() => setBriefExpanded(true)}
                            className="text-primary text-[10px] hover:text-primary/80 transition-colors font-medium">
                            See full brief…
                          </button>
                        </>
                      )}
                ...
                {/* Moment media assets — grouped by type, only populated groups shown */}
                {mediaAssets && mediaAssets.length > 0 && (() => {
                  const groups: { type: MediaAsset['type']; label: string; Icon: React.ElementType }[] = [
                  ...
                  const populated = groups.filter(g => mediaAssets.some(a => a.type === g.type))
                  ...
                  return (
                    <div className="bg-primary/5 border border-primary/20 rounded-xl p-3 space-y-3">
                      <p className="text-[10px] font-semibold text-primary uppercase tracking-wide flex items-center gap-1.5">
                        <Image className="h-3 w-3" /> Suggested media
                      </p>
                      {populated.map(({ type, label, Icon }) => (
                        <div key={type} className="space-y-1.5">
                          <p className="text-[10px] text-muted-foreground font-medium">{label}</p>
                          {mediaAssets.filter(a => a.type === type).map((asset, i) => (
                            <a key={i} href={asset.url} target="_blank" rel="noopener noreferrer"
                              className="flex items-center gap-2 p-2 bg-background rounded-lg border border-border hover:border-primary/40 transition-all text-xs text-foreground hover:text-primary group">
                              <Icon className="h-3 w-3 shrink-0 text-muted-foreground group-hover:text-primary transition-colors" />
                              <span className="truncate flex-1">{asset.caption || asset.url}</span>
                              <ExternalLink className="h-2.5 w-2.5 text-muted-foreground/50 shrink-0" />
                            </a>
                          ))}
                        </div>
            ...
            {/* ── Center: Generated copy ── */}
            <div className="flex-1 flex flex-col overflow-y-auto border-r border-border">
              <div className="p-6 flex flex-col flex-1 gap-4">
                ...
                {/* Voice indicator */}
                <div className="flex items-center gap-2 text-xs text-muted-foreground bg-secondary rounded-xl p-3 mt-auto">
              ...
              <div className="p-4 space-y-5">
                ...
                <div className="space-y-3">
                  ...
                  <ToneSlider
                    ...
                    left="Generic"
                    right="You"
                    value={voiceInfluenceScore}
                    onChange={setVoiceInfluenceScore}
